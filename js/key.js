    ...
    if ([33, 34, 37, 38, 39, 40, 122].includes(e.keyCode) || ["Cancel"].includes(e.key)) {
        ...
        if ([33, 34, 37, 38, 39, 40].includes(e.keyCode)) {
            ...
            if (App.deadwood?.isActive?.()) {
                App.deadwood.stop("OPERATOR DROP OR KILL.");
        ...
        if (e.keyCode === 67) { // Ctrl+C / Cmd+C
            ...
            if (!App.wasm.isRunning && !App.wasm.isLoading && !App.cmdrun && !App.deadwood?.isActive?.()) {
                Term.clearScreen();
                Term.prompt();
            }
...
Term.handleTab = () => {
    ...
    if (App.deadwood?.isActive?.()) {
        completion = App.deadwood.autocomplete?.(Term.input);
    } else if (!App.wasm.isRunning) {
        completion = Command.autocomplete?.(Term.input);
    }

    if (!completion || completion === Term.input.toUpperCase()) {
        return;
    }

    while (Term.input.length > 0) {
        Term.write("\b \b");
        Term.input = Term.input.slice(0, Term.input.length - 1);
    }
