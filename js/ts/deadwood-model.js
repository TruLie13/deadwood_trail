...
(function (DeadwoodModel) {
    ...
    function assessCrewConsequence(crew, visibleMorale, visibleFear, week = 0, _mutinyPressureState = 0) {
        ...
        const moraleActionScale = visibleMorale >= 90 ? 0 :
            visibleMorale >= 75 ? 0.35 :
                visibleMorale >= 60 ? 0.65 :
                    visibleMorale >= 45 ? 1 :
                        1.25;
        function personalBurden(member) {
        ...
        function privilegeScore(member) {
        ...
        const burdenedMembers = burdenEntries.filter(entry => entry.burden >= 6 ||
            entry.member.morale <= 24 ||
            entry.member.fear >= 82 ||
            entry.member.hunger >= 68);
        const requiredBurdenedCount = grievancePool.length <= 2 ? 1 : 3;
        const deserterCandidate = [...burdenEntries].sort((left, right) => right.burden - left.burden ||
            left.member.morale - right.member.morale ||
            right.member.fear - left.member.fear)[0];
        const privilegedCandidate = [...burdenEntries].sort((left, right) => right.privilege - left.privilege ||
            right.member.foodReceivedTotal - left.member.foodReceivedTotal ||
            right.member.whiskeyReceivedTotal - left.member.whiskeyReceivedTotal)[0];
    ...
    function damnedTradeCost(tradeTime, item, totalDealsUsed, itemPurchases) {
