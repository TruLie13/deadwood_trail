...
Term.writelns = async text => {
    const width = Math.max(24, (Term.cols || 80) - 2);
    const wrapped = [];
    ...
    for (const rawLine of lines) {
        ...
        const hanging = indent.length >= Math.min(4, width - 8) ? indent : indent + "  ";
        ...
        for (const word of words) {
            ...
            if ((current + prefix + word).length > width) {
    ...
    await Term.writes(wrapped.join("\n") + "\n");
};
